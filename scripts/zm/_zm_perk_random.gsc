...
function machine_think()
{
	...
	while( 1 )
	{
		...
		while( 1 )
		{
			...
			self thread perk_bottle_motion( player ); // bottle moves into place
...
function perk_random_vending( player )
{
	...
	if ( player HasPerk( PERK_SLEIGHT_OF_HAND ) )
	{
		self SetZBarrierPieceState(ZM_PERK_RANDOM_BOTTLE_TAG_PIECE_INDEX, "opening", 0.25); //move tag origin for bottle
	}
	else
	{
