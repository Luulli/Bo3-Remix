...
function dog_round_spawning()
{
	...
	while( true )
	{
		...
		while( zombie_utility::get_current_zombie_count() >= num_player_valid * 4 )
		{
			wait( 2 );
			num_player_valid = zm_utility::get_number_of_valid_players();
		}
...
function dog_health_increase()
{
	...
	{
		level.dog_health = 2400;
	}
	else if( level.dog_round_count == 4 )
	{
		level.dog_health = 2800;
	}

	if( level.dog_health > 2800 )
	{
		level.dog_health = 3500;
	}
}
