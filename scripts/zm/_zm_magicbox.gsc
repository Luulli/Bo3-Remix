...
//#precache( "fx", "custom/magic_box_og/fx_weapon_box_idle_glow_og" );
...
function __init__()
{
	...
	//level._effect["idle_glow"] 										= "custom/magic_box_og/fx_weapon_box_idle_glow_og";
...
function default_pandora_fx_func( )
{
	...
	if ( IsDefined( self ) && IsDefined( self.pandora_light ) )
	{
		...
		//playfxontag(level._effect["idle_glow"], self.origin, "tag_origin");
		//playfxontag(level._effect["idle_glow"], self.pandora_light, "tag_origin");
	}
...
{
    players = GetPlayers();
    ran = randomintrange(1,7);
    ran2 = randomintrange(1,7);
	if(ran == 3) //&& players.size == 1 )
	{
		//IPrintLnBold("ran 3");
		...
		{   
			if(level.round_number > 13 )
			{
			 ...
			 if(!player zm_weapons::has_weapon_or_upgrade( GetWeapon( "idgun_0" ) ) )
				return GetWeapon( "idgun_0" );
			}
		...
		{
			if(!player zm_weapons::has_weapon_or_upgrade( GetWeapon( "elemental_bow" ) ) )
				return GetWeapon( "elemental_bow" );
		...
		{
			if(!player zm_weapons::has_weapon_or_upgrade( GetWeapon( "hero_mirg2000" ) ) )
				return GetWeapon( "hero_mirg2000" );
		...
		{
		    if(!player zm_weapons::has_weapon_or_upgrade( GetWeapon( "raygun_mark3" ) ) )
				return GetWeapon( "raygun_mark3" );

			if(!player zm_weapons::has_weapon_or_upgrade( GetWeapon( "dragon_gauntlet_flamethrower" ) ) )
				return GetWeapon( "dragon_gauntlet_flamethrower" );
		...
		if(level.script == "zm_genesis")
		{
			...
			if(!player zm_weapons::has_weapon_or_upgrade( GetWeapon( "hero_gravityspikes_melee" ) ) )
				return GetWeapon( "hero_gravityspikes_melee" );
		...
		{	
			if(!player zm_weapons::has_weapon_or_upgrade( GetWeapon( "raygun_mark2" ) ) )
				return GetWeapon( "raygun_mark2" );

            if(!player zm_weapons::has_weapon_or_upgrade( GetWeapon( "cymbal_monkey" ) ) )
				return GetWeapon( "cymbal_monkey" );

			if(!player zm_weapons::has_weapon_or_upgrade( GetWeapon( "lmg_mg08" ) ) )
				return GetWeapon( "lmg_mg08" );
		...
		if(level.script == "zm_temple")
		{
			...
			if(!player zm_weapons::has_weapon_or_upgrade( GetWeapon( "shrink_ray" ) ) )
				return GetWeapon( "shrink_ray" );
		...
		if(level.script == "zm_asylum" || level.script == "zm_sumpf")
		{
			
			if(!player zm_weapons::has_weapon_or_upgrade( GetWeapon( "tesla_gun" ) ) )
				return GetWeapon( "tesla_gun" );
		...
		if(level.script == "zm_prototype" || level.script == "zm_daybreak" || level.script == "zm_theater"  || level.script == "hoch_oben" || level.script == "zm_eingesperrt")
		{
				
			if(!player zm_weapons::has_weapon_or_upgrade( GetWeapon( "thundergun" ) ) )
				return GetWeapon( "thundergun" );
		...
		if(level.script == "zm_cosmodrome")
		{
			...
			if(!player zm_weapons::has_weapon_or_upgrade( GetWeapon( "black_hole_bomb" ) ) )
				return GetWeapon( "black_hole_bomb" );
		...
		if(level.script == "zm_moon")
		{
			...
			if(!player zm_weapons::has_weapon_or_upgrade( GetWeapon( "microwavegun" ) ) )
				return GetWeapon( "microwavegundw" );
			...
			if(!player zm_weapons::has_weapon_or_upgrade( GetWeapon( "quantum_bomb" ) ) )
				return GetWeapon( "quantum_bomb" );
		...
		if(level.script == "zm_town" || level.script == "zm_diner" || level.script == "zm_farm_hd")
		{
		...
		if(level.script == "zm_coast")
		{
				
			if(!player zm_weapons::has_weapon_or_upgrade( GetWeapon( "humangun" ) ) )
				return GetWeapon( "humangun" );

			if(!player zm_weapons::has_weapon_or_upgrade( GetWeapon( "ray_gun" ) ) )
				return GetWeapon( "ray_gun" );

			if(!player zm_weapons::has_weapon_or_upgrade( GetWeapon( "nesting_dolls" ) ) )
				return GetWeapon( "nesting_dolls" );
...
//	model moveto( model.origin +( 0, 0, floatHeight ), 3, 2, 0.9 ); 
	...
	if(isdefined(chest.zbarrier))
	{
		...
		{
			chest.zbarrier thread magic_box_do_weapon_rise(player);
		}
	}

	for( i = 0; i < number_cycles; i++ )
	{
		if( player HasPerk( PERK_SLEIGHT_OF_HAND ) )
		{
			i += 3;
		}
...
function magic_box_do_weapon_rise(player)
{
	self endon("box_hacked_respin");
		...
		foreach( chest in level.chests ) 
		{
			if ( !IS_TRUE( chest.hidden ) ) 
			{
				if ( IsDefined( chest ) && IsDefined( chest.pandora_light ) )
				{
					...
					playfxontag(level._effect["idle_glow"], chest.origin, "tag_origin");
				}
			}
			util::wait_network_frame();
		}
