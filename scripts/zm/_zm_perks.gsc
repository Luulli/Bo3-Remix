...
#namespace zm_perks;
function explosive_increase(inflictor, attacker, damage, flags, mod, weapon, vpoint, vdir, sHitLoc, psOffsetTime, boneIndex, surfaceType) 
{
    if(IsPlayer(attacker) && zm_utility::is_explosive_damage( mod )) 
        return damage * 2;

    return -1;
}

function explosive_immunity(inflictor, attacker, damage, flags, mod, weapon, vpoint, vdir, sHitLoc, psOffsetTime, boneIndex, surfaceType) 
{

    if(IsPlayer(self) && self HasPerk(PERK_WIDOWS_WINE) && (zm_utility::is_explosive_damage( mod ) || mod == "MOD_FALLING")) 
        return 0;

    return -1;
}
// self == zombie
function deadshot_headshot_increase(inflictor, attacker, damage, flags, mod, weapon, vpoint, vdir, sHitLoc, psOffsetTime, boneIndex, surfaceType)
{
    ...
    if(zm_utility::is_headshot(weapon, sHitLoc, mod) && attacker HasPerk("specialty_deadshot"))
    {
        return damage * 2;
    }
...
function shangexplosive_immunity(inflictor, attacker, damage, flags, mod, weapon, vpoint, vdir, sHitLoc, psOffsetTime, boneIndex, surfaceType) 
{
	if (level.script == "zm_temple" || level.script == "zm_factory")
	{
		 if(IsPlayer(self) && (zm_utility::is_explosive_damage( mod ) || mod == "MOD_FALLING")) 
          return 0;
...
function init()
{
	...
	zm::register_player_damage_callback(&explosive_immunity);
	zm::register_actor_damage_callback(&explosive_increase);
	zm::register_actor_damage_callback(&deadshot_headshot_increase);
	zm::register_player_damage_callback(&shangexplosive_immunity);
...
function give_perk( perk, bought )
{
	...
	if( IS_TRUE(bought) )
	{
     ...
     if( perk == "specialty_sleight_of_hand" )
	 {
		self setperk("specialty_overcharge");
		//IPrintLnBold("overcharge");
	 }
...
function quantum_bomb_give_nearest_perk_result( position )
{
	...
	for ( i = 0; i < players.size; i++ )
	{
		...
		if ( !player HasPerk( perk ) && ( !isdefined( player.perk_purchased ) || player.perk_purchased != perk) ) // 5 = 80% chance
		{
			if( player == self )
			{
